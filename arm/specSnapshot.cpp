...
/*
 * Snapshot bootstrapper (re)stores register content from SNA header loaded on #8000.
 * #8040: restore context
 * #8080: save context
 */
const unsigned char snapshotBootstrap[] = {
	
};
...
bool LoadSnapshot(const char *fileName)
{
	...
	if (f_open(&snaFile, fileName, FA_READ) == FR_OK) {
		if (snaFile.fsize >= 0xc01b) {
			...
			addr = (0x02 << 14) | (0x00);
			for (i = 0; i < 0x1b; i++)
				SystemBus_Write(addr++, header[i]);
			addr = (0x02 << 14) | (0x40);
			for (i = 0; i < sizeof(snapshotBootstrap); i++)
				SystemBus_Write(addr++, snapshotBootstrap[i]);
...
*/
		...
		addr = (0x02 << 14) | (0x00);
		for (i = 0; i < 0x40 + sizeof(snapshotBootstrap); i++) {
			byte data;

			if (f_read(&snaFile, &data, 1, &res) != FR_OK)
				break;
			if (res == 0)
				break;
