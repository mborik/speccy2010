...
void hide_sel()
{
	...
	if (files_size != 0) {
		...
		if (fr.sel)
			WriteChar(selx * 16, 2 + sely, 0x95);
		else
			WriteChar(selx * 16, 2 + sely, ' ');
	}
...
bool Shell_CopyItem(const char *srcName, const char *dstName, bool move = false)
{
	...
	if (move)
		Shell_MessageBox("Error", "Cannot move/rename to", shortName, GetFatErrorMsg(res));
	else
		Shell_MessageBox("Error", "Cannot copy to", shortName, GetFatErrorMsg(res));
...
bool Shell_Copy(const char *_name, bool move)
{
	...
	if (files_sel_number == 0 || !newPath) {
		...
		if (newPath)
			...
			sniprintf(pnameNew, sizeof(pnameNew), "%s%s", path, name.String());
...
void CMenuItem::Redraw()
{
	...
	WriteStr(x2, y, data);
	if (state == 0)
		WriteAttr(x2, y, 0x47, size);
	else if (state == 1)
		WriteAttr(x2, y, 0x39, size);
	else
		WriteAttr(x2, y, 0x40 | 0x10 | 0x00, size);
}
