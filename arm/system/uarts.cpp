...
void CUart::IRQHandler()
{
	...
	while (UART_GetITStatus(uart, UART_IT_Transmit) == SET) {
		if (tx_cntr) {
			...
			if (tx_read_ptr >= tx_size)
				tx_read_ptr = 0;
			tx_cntr--;
		}
		else
			UART_ClearITPendingBit(uart, UART_IT_Transmit);
	}
}
//---------------------------------------------------------------------------------------
dword CUart::ReadFile(byte *s, dword N)
{
	portENTER_CRITICAL();
	if (N > rx_cntr)
		N = rx_cntr;
